...
addresses_tbl <- tibble::tibble(recId = seq_len(length(addresses)),
...
# Parse results----
result_parsed <- tibble::as_tibble(read.table(text = result_content,
                                              header = TRUE, sep = "|"))
...
# param management----
limit = 1
outcols <- c('displayLatitude', 'displayLongitude', 'locationLabel', 'street',
...
test3 <- tidygeocoder::geo(address = c("------","Barcelona"),
                           method = "here",
                           verbose = TRUE,
                           full_results = TRUE,
                           mode="batch",
                           limit = 1,
                           custom_query = list(indelim=";",
                                               outdelim=":"))

test3

test4 <- tidygeocoder::geo(address = c("Calle Mayor, Madrid, Spain"),
                           method = "here",
                           verbose = FALSE,
                           full_results = TRUE,
                           mode="batch",
...
test5 <- tidygeocoder::geo(address = c("Gran Via, Madrid, Spain"),
                           method = "here",
                           verbose = FALSE,
                           full_results = TRUE,
                           mode="batch",
