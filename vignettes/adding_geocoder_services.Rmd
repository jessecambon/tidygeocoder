...
description: >
  Refer to this page on how to add support for a geocoder service to the package.
vignette: >
...
The two core functions in the package are [geo()](https://github.com/jessecambon/tidygeocoder/blob/master/R/geo.R) and [reverse_geo()](https://github.com/jessecambon/tidygeocoder/blob/master/R/reverse_geo.R). They are both have a similar layout, but `geo()` is for forward geocoding while `reverse_geo()` is for reverse geocoding. The both take inputs (either addresses or coordinates) and call other functions as needed to deduplicate the inputs and perform geocoding queries. Key parameters and settings are stored in several in-built datasets which are documented below.
...
* Run [external_tests/online_tests.R](https://github.com/jessecambon/tidygeocoder/blob/master/external_tests/online_tests.R) which will test making queries to the geocoder services. These tests are not included internally because they require API keys which would not exist on all systems and are dependent on the geocoder services being online at that the time of the test.
* Run the commands detailed in [cran-comments.md](https://github.com/jessecambon/tidygeocoder/blob/master/cran-comments.md) to test the package on other environments.