...
The two core functions in the package are [geo()](https://github.com/jessecambon/tidygeocoder/blob/master/R/geo.R) and [reverse_geo()](https://github.com/jessecambon/tidygeocoder/blob/master/R/reverse_geo.R). They both have a similar layout, but `geo()` is for forward geocoding while `reverse_geo()` is for reverse geocoding. Both take inputs (either addresses or coordinates) and call other functions as needed to deduplicate the inputs and perform geocoding queries. Key parameters and settings for geocoding are stored for easy access and display in in-built datasets which are documented below.
