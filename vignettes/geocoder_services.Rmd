...
This page documents the supported geocoder services and also displays the built-in datasets the package uses to set query parameters and settings.

## Overview

The supported geocoder services are shown in the table below along with if they require an API key and support batch geocoding. The "method" is used to specify the geocoder service in tidygeocoder functions such as `geo()` and `reverse_geo()`. Refer to each service's API documentation for more details.
