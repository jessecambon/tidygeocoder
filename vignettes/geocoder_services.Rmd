...
Due diligence must be exercised when geocoding sensitive data as tidygeocoder utilizes third party web services to perform geocoding. Within the context of healthcare, using patient or study subject address data with a third party geocoding service risks violating standard IRB and HIPAA privacy rules. 
...
- The Geocodio service offers [a HIPAA compliant endpoint](https://www.geocod.io/healthcare/). To enable the use of this endpoint in tidygeocoder, you can use the `api_options=list(geocodio_hipaa=TRUE)` parameter.
- The Nominatim service (`method="osm"`) can be installed and hosted locally so that data does not leave your network. See the [Nominatim website](https://nominatim.org/) for instructions on how to host your own service. You can use a locally hosted Nominatim service with tidygeocoder by specifying its address with the `api_url` parameter.
