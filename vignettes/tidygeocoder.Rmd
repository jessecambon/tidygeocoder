...
The `cascade` method first tries to use one geocoding service and then again attempts to geocode addresses that were not found using a second geocoding service. By default it first uses the Census Geocoder and then OSM, but you can specify any two methods you want (in order) with the `cascade_order` argument.
