...
Geocoding services are used to provide data about addresses such as longitude and latitude coordinates. The goal of tidygeocoder is to make getting data from these services easy. The two main functions to use are `geocode()` which takes a dataframe as an input and `geo()` which takes character values.
...
The `geocode()` function takes a dataframe input with address data. Use the `address` argument for single line addresses:

```{r}
address_single %>% geocode(address = singlelineaddress, method = 'census')
```

Alternatively you can run the same query with the `geo()` function by passing the address values from the dataframe directly. Additionally, `verbose = TRUE` displays logs to the console.

```{r}
geo(address = address_single$singlelineaddress, method = 'census', verbose = TRUE)
```
...
```{r}
address_components %>% geocode(street = str, city = cty, state = st, method = 'census',  verbose = TRUE)
```
...
addr_comp1 %>% geocode(street = str, state = st, city = cty, country = country, method = 'cascade')
```
...
```{r}
salz <- geo('Salzburg, Austria', method = 'osm', full_results = T)
glimpse(salz)
```

## Advanced Usage

Here are some advanced usage notes for the `geocode()` and `geo()` functions.

* To specify a custom Geocoder API URL, use the `api_url` argument. Also, the `iq_region` and `geocodio_v` arguments are helper functions for customizing the API URL. 
* By default batch geocoding will be used if available when more than one address is passed, otherwise single address geocoding will be used. To ovveride this default, use the `mode` argument.
* API parameters can be passed directly using the `custom_query` parameter.
* Refer to the `api_parameter_reference` dataset and the `get_api_query` functions for more details on how geocoding queries are constructed. 