...
```{r}
address_single %>% geocode(address = singlelineaddress, method = 'census', verbose = TRUE)
```

Alternatively you can run the same query with the `geo()` function by passing the address values from the dataframe directly.
...
```{r}
geo(c('Lima, Peru', 'Cairo, Egypt'), method = 'osm', limit = 3, full_results = TRUE)
```
...
* Refer to the `api_parameter_reference` dataset and the `get_api_query()` and `query_api()` functions for more details on how geocoding queries are constructed.