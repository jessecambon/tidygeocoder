...
Geocoder services are used to provide data (such as latitude and longitude coordinates) about locations (**forward geocoding**) or to provide data about the location of specific latitude and longitude coordinates (**reverse geocoding**). The goal of tidygeocoder is to make getting data from these services easy. For working with dataframes, you can use the `geocode()` and `reverse_geocode()` functions. To pass input values (addresses or latitude and longitude coordinates, you can alternatively use the `geo()` and `reverse_geo()` functions. 

**For forward geocoding**, the `geocode()` function extracts specified address columns from the input dataframe and passes them to `geo()`. All extra arguments (`...`) given to `geocode()` are passed to `geo()` so refer to the documentation in `geo()` for all the possible arguments you can give to the `geocode()` function.

**For reverse geocoding**, the `reverse_geocode()` function extracts latitude and longitude values and passes them to `reverse_geo()`. All extra arguments (`...`) given to `reverse_geocode()` are passed to `reverse_geo()` so refer to the documentation in `reverse_geo()` for all the possible arguments you can give to the `reverse_geocode()` function.
...
Reverse geocoding is structured in a very similar fashion to forward geocoding and many of the parameters that are used are the same. However, instead of `geocode()` you'll use `reverse_geocode()` and instead of `geo()` you'll use `reverse_geo()`. Latitude and longitudes are provided as inputs and the address is returned. 
...
```{r}
lat_lngs1 <- tibble(latitude = c(38.895865, 43.6534817), 
