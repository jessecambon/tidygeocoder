...
The `geocode_combine()` function allows you to easily combine the results of multiple queries. The queries are executed in the order provided. By default only addresses that are not found are passed to the next service, but this behavior can be toggled with the `cascade` argument.

```{r}
cascade_results1 <-  tibble(
