...
By default, as shown above, duplicates will not be removed from your results. However, if you chose you can only return unique results with the `unique_only` argument. Note that when used with `geocode()` passing `unique_only = TRUE` will result in the original dataframe format (including column names) being discarded in favor of the standard field names (ie. address, city, state, etc.).
...
When the `api_name` is missing that means the given parameter is not supported by the given geocoder service. When `generic_name` is missing (such as for the vintage and benchmark parameters for the census) that means the given parameter is specific to that geocoding service. 
