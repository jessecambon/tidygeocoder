...
Note that international and non-street addresses were not found since we are using the US Census geocoder service. 

```{r map,fig.width=8,fig.height=5,warning=F}
...
To find international and non-street addresses, we can use the OSM service. The 'cascade' method will attempt to use the US Census method for each address and only use the OSM service if the Census method fails (since OSM has a usage limit).
