...
Tidygeocoder makes getting data from geocoder services easy. A unified interface is provided for the geocoder services listed below. 

**Features:**

- Both **forward geocoding** (addresses -> coordinates) and **reverse geocoding** (coordinates -> addresses) are supported. 
- All results are returned in [tibble](https://tibble.tidyverse.org/) (dataframe) format.
...
geocoder_summary_table <-
  tidygeocoder::api_info_reference %>%
    ...
    select(service, method, api_key_required, batch_geocoding, usage_limitation = description, api_documentation)
...
geocoder_summary_table %>%
  knitr::kable()
```
**Note that the US Census service is limited to the United States while Geocodio is limited to the US and Canada.**
