...
Tidygeocoder makes getting data from geocoding services easy. The currently supported services are the [US Census](https://geocoding.geo.census.gov/) geocoder, [Nominatim (OSM)](https://nominatim.org), [Geocodio](https://www.geocod.io/), and [Location IQ](https://locationiq.com/). Batch geocoding (geocoding multiple addresses per query) is used by default for the US Census and Geocodio services when given multiple addresses. Also only unique addresses are passed to geocoder services.

In addition to the usage example below, you can find a blog post I wrote up on geocoding landmarks in Washington, DC [here](https://jessecambon.github.io/2019/11/11/tidygeocoder-demo.html).
