...
-   `geocode()` and `reverse_geocode()` now require `limit = 1` (default) unless `return_inputs = FALSE`. This fixes a bug where geocoding results could be misaligned when `limit > 1`.  ([#88](https://github.com/jessecambon/tidygeocoder/issues/88)).
