...
- Added the ability to set default arguments with `options()` for `verbose`, `quiet`, `flatten`, `progress_bar`, and `full_results`. For instance `options(tidygeocoder.full_results = TRUE)` changes the default of `full_results` from FALSE to TRUE.
