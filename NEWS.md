...
### Tweaks
-   `geocode()` and `reverse_geocode()` will now require the `limit` argument to be set to 1 (default) unless `unique_only = TRUE` or `return_addresses`/`return_coords` is set to TRUE. This fixes a bug where geocoding results could be misaligned when `limit > 1`.  ([#88](https://github.com/jessecambon/tidygeocoder/issues/88)).
