...
batch_tomtom <- function(unique_addresses, lat = "lat", long = "long",
  ...
  # flatten is not passed here because we are just going to extract
  # the results dataframe from the nested structure
...
batch_mapquest <- function(unique_addresses, lat = "lat", long = "long", method = "mapquest", flatten = TRUE,
  ...
  if (nrow(address_df) == 1) {
    results <- geo(
      address = address_df[["address"]], method = method,
