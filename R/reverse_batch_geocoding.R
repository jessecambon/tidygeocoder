...
reverse_batch_geocodio <- function(lat, long, timeout = 20, full_results = FALSE, custom_query = list(),
verbose = FALSE, api_url = NULL, geocodio_v = 1.6, limit = 1, ...) {
  ...
  if (full_results == FALSE)  return(results[c('formatted_address')])
  else return(cbind(results[c('formatted_address')], results[!names(results) %in% c('formatted_address')]))
}
